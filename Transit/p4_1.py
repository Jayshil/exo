...
pout = os.getcwd() + '/Transit/Analysis/NumPyro_Real_luas'
cpu_cores = 2
...
def model():
    ...
    ar = numpyro.sample('ar', dist.Uniform(low=4., high=6.))
    ...
    # Transit light curve
    orbit = KeplerianOrbit.init(time_transit=tc, period=per, semimajor=ar, impact_param=bb, radius=rprs)
    light_curve = QuadLightCurve.init(u1=u1, u2=u2)
    transit_model = 1 + light_curve.light_curve(orbit, tim)[0]
...
all_var_names = ['tc', 'ar', 'bb', 'rprs', 'u1', 'u2', 'mflx', 'sig_w',\
...
svi_result = svi.run(PRNGKey(5), 5000)#, Y)
...
sampler = NUTS(model, dense_mass=True,\
               ...
               init_strategy=numpyro.infer.init_to_value(values=p_fit))

# Monte Carlo sampling for a number of steps and parallel chains:
mcmc = MCMC(sampler, num_warmup=4_000, num_samples=4_000, num_chains=cpu_cores)
...
total_model = np.array(result.posterior['total']).reshape(8000, len(tim))
gp_model = np.array(result.posterior['gp']).reshape(8000, len(tim))
...
for i in range(50):
    axs.plot(tim, total_model[np.random.choice(np.arange(6000), replace=False),:]+gp_model[np.random.choice(np.arange(8000), replace=False),:], 'r-', alpha=0.1, zorder=10)
